...
const AcMaintenanceAdd = ({routes, navigation}) => {
  ...
  const submitHandle = async (e) => {
    ...
    formData.append('comment', price);
    formData.append('user_id', user.id);

    await axios.post(`${BASE_URL}acmaintenance/add`, formData,
          ...
          <ScrollView>
            <View style={{paddingBottom: 100}}>
              <View style={{flexDirection: 'column', justifyContent: 'center', padding: 20}}>
                ...
                <View style={{flexDirection: 'row', justifyContent: 'flex-start'}}>
                  <Text style={{width: '50%', marginTop: 15}}>{t('Number of AC units')}</Text>
                  <TextInput
                ...
                <View style={{marginTop: 15, marginTop: 15}}>
                  <Text style={{flexDirection: 'row', alignSelf: 'center'}}>Regular AC Cleaning (only AED 75 per unit) includes cleaning filters, vents and water trays and general maintenance of the external unit.</Text>
                </View>
...
export default AcMaintenanceAdd;