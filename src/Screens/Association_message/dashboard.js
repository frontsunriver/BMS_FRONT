...
const MessageDashboard = ({route, navigation}) => {
  ...
  useEffect( async () => {
    ...
    await axios.post(`${BASE_URL}/messages/getList`).then( res => {
      if(isMounted) {
        if(res.data.success) {
  ...
  return (
      <LoadingActionContainer fixed>
        <Container
          ...
          <AssociationFooterScreen tabIndex={4}/>
        </Container>
...
const MessageListItem = (props) => {
    ...
    const showDashboardDetail = () => {
      navigation.navigate(Routes.ASSOCIATION_MESSAGES_DETAIL_SCREEN, {data: data});
    }
    return (
