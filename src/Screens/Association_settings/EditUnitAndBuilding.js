...
const EditBuildingAndUnit = ({route, navigation}) => {
  const {t} = useTranslation();
  const {theme} = useAppTheme();
  const {data} = route.params;
  const [dropdown, setDropdown] = useState(data.building_id);
  ...
  const submitHandle = async (e) => {
    ...
    await axios.post(`${BASE_URL}userowner/update`, formData,
    { headers: { 'Content-Type': 'multipart/form-data', 'X-Requested-With': 'XMLHttpRequest', }}).then(res => { 
      if(res.data.success) {
        showSuccessToast('Successfully add');
        setDropdown(null);
...
export default EditBuildingAndUnit;
