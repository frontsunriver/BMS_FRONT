...
const MaintenanceEdit = ({route, navigation}) => {
  const {t} = useTranslation();
  const {theme} = useAppTheme();
  const {data} = route.params;
  const [user, setUser] = useState({});
  const [dropdown, setDropdown] = useState(null);
  const [unitDropDown, setUnitDropDown] = useState(data.unit_id);
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
  const [dateString, setDateString] = useState(new Date(data.move_date));
  const [buildingData, setBuildingData] = useState([]);
  const [unitData, setUnitData] = useState([]);
  const [singleFile, setSingleFile] = useState(null);
  const [carried_content, setCarried_content] = useState(data.carried_content);
  ...
  const submitHandle = async (e) => {
    ...
    await axios.post(`${BASE_URL}move/maintenacneUpdate`, formData,
