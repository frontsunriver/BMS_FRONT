...
const MoveInEdit = ({route, navigation}) => {
  ...
  const [dateString, setDateString] = useState(new Date(data.move_date))
  ...
  const [name, setName] = useState(data.tenants_name);
  const [email, setEmail] = useState(data.tenants_email);
  const [mobile, setMobile] = useState(data.tenants_mobile);
  ...
  const submitHandle = async (e) => {
    ...
    formData.append('owner_passport', singleFile);
    ...
    await axios.post(`${BASE_URL}move/updateMoveIn`, formData,
